...
class AgentGateway:
    ...
    def _handle_chat_fastpath(self, req: ChatRequest, routed: Any) -> ChatResponse:
        ...
        try:
            chat_req = req.model_copy(update={"session_id": session_id})
            if not tool_names and not tool_schemas:
                msgs = list(getattr(req, "messages", []))
                if msgs and not any((getattr(m, "role", None) == "system" or (isinstance(m, dict) and m.get("role") == "system")) for m in msgs):
                    system_msg = {"role": "system", "content": "You are a helpful assistant. Answer the user directly and concisely."}
